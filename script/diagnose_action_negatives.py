"""Diagnose type-valid hard negatives around reference actions."""
...
from action_negative_sampling import NegativeActionExample, sample_action_negatives
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--label-applicability", action="store_true")
...
def main() -> int:
    ...
    for transition_index, (trajectory, record, step_idx, true_action) in enumerate(
        ...
        space = space_cache[trajectory.problem_index]
        applicable: set[tuple[str, tuple[str, ...]]] | None = None
        if args.label_applicability:
        ...
        def label(action: Any) -> bool:
            assert applicable is not None
            return action_key(action) in applicable

        examples = sample_action_negatives(
            space,
            true_action,
            per_category=args.per_category,
            seed=args.seed + transition_index,
            applicability_fn=label if applicable is not None else None,
        )
        ...
        details.append(
            {
                ...
                "negatives": [_example_payload(example) for example in examples],
            }
        )
    ...
    summary = {
        ...
        "metrics": {
            ...
            "negatives": sum(category_counts.values()),
            "labeled_applicability": args.label_applicability,
...
def _example_payload(example: NegativeActionExample) -> dict[str, Any]:
    return {
        "action": action_payload(example.action),
        "category": example.category,
        "changed_roles": list(example.changed_roles),
